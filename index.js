...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('Ꭰᥲʀκ͢ BOT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Ꭰᥲʀκ͢', 'yellow'))
	...
	await aruga.sendText(chat.id, `Sorry, grup di bot ini sudah penuh\nMax Group is: ${groupLimit}`).then(() => {
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
	    ...
	    await aruga.sendText(chat.id, `Sorry, Ꭰᥲʀκ͢ BOT keluar jika anggota grup tidak melebihi ${memberLimit} people`).then(() => {
	      aruga.leaveGroup(chat.id)
	      aruga.deleteChat(chat.id)
	    })
	    } else {
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai Gan~, Saya adalah Ꭰᥲʀκ͢ Bot. Terimakasih sudah menginvite bot ini, untuk melihat menu silahkan kirim ${prefix}menu`)
        })
	    }
	}
    })
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'remove' && event.who !== host) {
			await aruga.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
            await aruga.sendTextWithMentions(event.chat, `Selamat tinggal @${event.who.replace('@c.us', '')}, We'll miss you✨`)
        }
    })
