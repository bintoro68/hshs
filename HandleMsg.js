	...
	// Filter Banned People
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai Gan~, Saya adalah Ꭰᥲʀκ͢ Bot. Terimakasih sudah menginvite bot ini, untuk melihat menu silahkan kirim ${prefix}menu`)
                      })
            } else {
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    aruga.reply(from, Nih sticker lu dah jadi...')
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ...
                    : aruga.reply(from, 'Nih sticker lu dah jadi...')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
        ...
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                aruga.reply(from, 'Proses gan..', id)
                try {
                    const hasilqmaker = await images.quote(quotes, author, theme)
                    aruga.sendFileFromUrl(from, `${hasilqmaker}`, '', 'Ini gan..', id)
                } catch {
                    aruga.reply('Yahh proses gagal, lu isinya sudah benar belum?..', id)
                }
            } else {
                aruga.reply(from, `Pemakaian ${prefix}quotemaker |isi quote|author|theme\n\ncontoh: ${prefix}quotemaker |Gw bot|Anker|random\n\nuntuk theme nya pakai random ya gan..`)
            }
            break
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', 'Nih gan...', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
		...
		case 'facebook':
			...
			.then(async (res) => {
				...
				await aruga.sendFileFromUrl(from, linkhd, '', 'Nih gan videonya', id)
				.catch(async () => {
					await aruga.sendFileFromUrl(from, linksd, '', 'Nih gan videonya', id)
					.catch(() => {
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomnimex, '', 'Ni gan...', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
        ...
        case 'tts':
            ...
            const dataText = body.slice(8)
                if (dataText === '') return aruga.reply(from, 'apa teksnya gan..', id)
                try {
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *Ꭰ ᥲ ʀ κ͢  B o t* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *Ꭰ ᥲ ʀ κ͢  B o t* 〙\n\n${msg}`)
            }
